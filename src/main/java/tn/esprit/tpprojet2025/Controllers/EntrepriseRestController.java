...
public class EntrepriseRestController {
    ...
    @Operation(description = "Récupérer les entreprises par adresse")
    @GetMapping("/getEntreprisesByAdresse/{adresse}")
    List<Entreprise> getEntreprisesByAdresse(@PathVariable("adresse") String adresse) {
        return entrepriseService.retrieveEntreprisesByAdresse(adresse);}
    @Operation(description = "Récupérer les entreprises par spécialité d'équipe")
    @GetMapping("/getEntreprisesBySpecialiteEquipe/{specialite}")
    List<Entreprise> getEntreprisesBySpecialiteEquipe(@PathVariable("specialite") String specialite) {
        return entrepriseService.retrieveEntreprisesBySpecialiteEquipe(specialite);}



    @Operation(description = "Mettre à jour l'adresse d'une entreprise")
    @PutMapping("/updateEntrepriseAdresse/{idE}/{adresse}")
    int updateEntrepriseAdresse(@PathVariable("idE") Long idEntreprise,
                               @PathVariable("adresse") String adresse) {
        return entrepriseService.updateEntrepriseByAdresse(adresse, idEntreprise);}
    @Operation(description = "Supprimer les entreprises par adresse")
    @DeleteMapping("/deleteEntreprisesByAdresse/{adresse}")
    int deleteEntreprisesByAdresse(@PathVariable("adresse") String adresse) {
        return entrepriseService.deleteEntrepriseByAdresse(adresse);}
}
